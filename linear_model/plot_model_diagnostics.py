...
MODEL_FILE = os.path.join(MODEL_DIR, 'linear_alpha_model.joblib')
PLOT_DIR = os.path.join(MODEL_DIR, 'plots')
# --- NEW: File to READ the selected features from ---
...
def load_data_and_model(data_path, model_path, feature_path):
    """
    Loads the data, model, and filters test data to the selected features.
    """
    ...
    try:
        ...
        print(f"ERROR: Could not find feature file: {feature_path}")
        print("Please run the training script first to generate this file.")
        return None, None, None
    except KeyError:
        print("ERROR: Feature file is in the wrong format. Expected a 'feature' column.")
        return None, None, None
    ...
    # --- FIX: Filter test set to only the features read from the file ---
    # This guarantees the columns and order match the model's training
    try:
        X_test_selected = X_test_full[selected_features]
    except KeyError:
        print("ERROR: The feature list in the CSV does not match columns in tsp_features.csv.")
        print("You may need to re-run the feature creator or training script.")
        return None, None, None
...
def plot_predicted_vs_actual(y_test, y_pred, save_path):
    ...
    plt.title('Predicted vs. Actual $\\alpha$ Values')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, '1_predicted_vs_actual.png'))
    plt.close()
...
def plot_residuals_vs_predicted(y_pred, residuals, save_path):
    ...
    plt.title('Residuals vs. Predicted $\\alpha$')
    plt.xlabel('Predicted $\\alpha$')
    plt.ylabel('Residuals (Actual - Predicted)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, '2_residuals_vs_predicted.png'))
    plt.close()
...
def plot_residual_histogram(residuals, save_path):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, '3_residual_histogram.png'))
    plt.close()
...
def plot_qq_plot(residuals, save_path):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, '4_qq_plot.png'))
    plt.close()
...
if __name__ == '__main__':
    ...
    model, X_test, y_test = load_data_and_model(DATA_FILE, MODEL_FILE, FEATURE_STATS_FILE)
